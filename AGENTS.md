...
- `time-tracking-app` (CI) should trigger on PRs and pushes; run `mvn -B clean verify`, `npm run lint`, and `npm run test`, aggregate coverage, and block merges on failures.
- Successful main-branch builds should produce versioned Docker images (backend + bundled frontend), publish build artifacts, and surface test/security reports.
...
- Test coverage: use JaCoCo + frontend coverage uploads sent to Codecov for PR diff reporting.
...
- `ci.yml` (PRs to `main`, pushes to `main`):
  - Jobs: `backend-build-test` (Maven verify + JaCoCo upload), `frontend-quality` (npm install/lint/test + coverage), `security-scans` (Trivy fs scan, Snyk SCA, OWASP Dependency-Check), `docker-build` (buildx build; push to GHCR only on `main`), `quality-gate` (final status aggregation, uploads reports/artifacts). Each job uses caching (`actions/setup-java` w/ Maven cache, `actions/setup-node` w/ npm cache) and uploads junit/coverage artifacts.
  - Conditional coverage upload via `codecov/codecov-action` in backend/frontend jobs; PR comments disabled, rely on status checks.
...
- Shared practices:
  ...
  - Enforce required checks in GitHub (backend, frontend, security, Codecov, docker build) before merge.
